"""
Absolute Cost Estimator: Embedding + KNN approach.

Given a query, estimates the absolute cost (in USD) for each model,
using KNN over embedded historical queries.
"""
...
class AbsoluteCostEstimator:
    """Estimates absolute costs across models using embedding-based KNN.

    Workflow:
        1. build_index(): load historical data, record per-query absolute costs,
           embed all queries, and build a KNN-ready index.
        2. estimate(query): embed the query, find K nearest neighbors,
           and return weighted-average absolute costs.
    """
    ...
    def __init__(
        ...
        self._costs: np.ndarray | None = None   # shape (n_queries, n_models), absolute costs in USD
        self._prompt_tokens: np.ndarray | None = None   # shape (n_queries, n_models)
        self._completion_tokens: np.ndarray | None = None  # shape (n_queries, n_models)
        self._embeddings: np.ndarray | None = None   # shape (n_queries, embed_dim)
        self._model_list: list[str] = []        # ordered model list
    ...
    def build_index(
    ...
    ) -> "AbsoluteCostEstimator":
        """Load data, record absolute costs, embed queries, build KNN index.
        ...
        # Step 2: Build cost matrix (absolute costs)
        self._compute_costs(query_data)
    ...
    def estimate(self, query: str) -> dict[str, float]:
        """Estimate absolute costs for a new query via KNN.

        Args:
            query: The input query string.

        Returns:
            Dict mapping model_name → estimated absolute cost (USD).
        """
        ...
        top_k_costs = self._costs[top_k_idx]  # (k, n_models)
        estimated = np.average(top_k_costs, axis=0, weights=weights)
    ...
    def estimate_batch(self, queries: list[str]) -> list[dict[str, float]]:
        """Estimate absolute costs for multiple queries.

        Args:
            queries: List of query strings.

        Returns:
            List of dicts, each mapping model_name → estimated absolute cost (USD).
        """
        if self._embeddings is None:
            raise RuntimeError("Index not built. Call build_index() first.")
        ...
        for i in range(len(queries)):
            ...
            weights = top_k_sims / top_k_sims.sum()
            top_k_costs = self._costs[top_k_idx]
            estimated = np.average(top_k_costs, axis=0, weights=weights)
    ...
    def model_list(self) -> list[str]:
        """Ordered list of models in the cost vector."""
    ...
    def _load_data(
        ...
        # Group by (dataset_prefix, record_index) → costs and tokens per model
        ...
        for dataset_prefix, model_name, records in records_by_file:
            ...
            for rec in records:
                ...
                query_data[key]["prompt_tokens"][model_name] = rec.get("prompt_tokens", 0)
                query_data[key]["completion_tokens"][model_name] = rec.get("completion_tokens", 0)
    ...
    def _parse_filename(self, stem: str) -> tuple[str, str] | None:
        ...
        # Fallback: use experiment config
        ...
        # Last resort: common model name patterns
        candidate_models = [
    ...
    def _compute_costs(self, query_data: dict[tuple, dict]) -> None:
        """Extract absolute costs and token counts per query per model.

        Populates self._queries, self._query_keys, self._costs,
        self._prompt_tokens, self._completion_tokens.
        Only includes queries where all models have positive cost.
        """
        queries = []
        keys = []
        costs = []
        ...
        for key in sorted(query_data.keys()):
            ...
            cost_dict = entry["costs"]

            # Must have all models present with positive cost
            if not all(m in cost_dict and cost_dict[m] > 0 for m in self._model_list):
                continue

            row = [cost_dict[m] for m in self._model_list]
            pt_row = [entry["prompt_tokens"].get(m, 0) for m in self._model_list]
            ct_row = [entry["completion_tokens"].get(m, 0) for m in self._model_list]
            queries.append(entry["query"])
            keys.append(key)
            costs.append(row)
            prompt_toks.append(pt_row)
            completion_toks.append(ct_row)

        self._queries = queries
        self._query_keys = keys
        self._costs = np.array(costs, dtype=np.float64)  # (n, n_models)
        self._prompt_tokens = np.array(prompt_toks, dtype=np.float64)
        self._completion_tokens = np.array(completion_toks, dtype=np.float64)
    ...
    def evaluate(
        ...
        absolute costs, then computes error against ground truth.
        ...
        train_costs = self._costs[train_idx]
        test_embs = self._embeddings[test_idx]
        test_costs = self._costs[test_idx]
        ...
        for i in range(len(test_idx)):
            ...
            if metric == "mae":
                ...
                errors[i] = np.abs(predicted - actual) / np.maximum(np.abs(actual), 1e-8)
            else:
    ...
    def mean_baseline(
        ...
        """Baseline: predict using the mean cost per model from the training set.

        This is query-independent — every test query gets the same prediction
        (the average cost of each model across the training set).
        ...
        # Mean cost per model from training set
        mean_costs = train_costs.mean(axis=0)  # (n_models,)

        # MAE: each test query gets the same prediction
        errors = np.abs(test_costs - mean_costs[None, :])  # broadcast
        mean_errors = errors.mean(axis=0)
        return {m: float(mean_errors[j]) for j, m in enumerate(self._model_list)}

    def evaluate_token_based(
        self,
        model_info: list[dict],
        test_ratio: float = 0.2,
        seed: int = 42,
    ) -> dict[str, float]:
        """Evaluate KNN by predicting tokens then computing cost via pricing.

        For each test query, finds KNN in the train set, predicts
        prompt_tokens and completion_tokens per model, then computes
        cost = prompt_tokens * input_price / 1M + completion_tokens * output_price / 1M.
        Reports MAE against actual cost.
        ...
        input_prices = np.array(
            [pricing[m][0] for m in self._model_list], dtype=np.float64
        )
        output_prices = np.array(
            [pricing[m][1] for m in self._model_list], dtype=np.float64
        )

        train_idx, test_idx = self.train_test_split(test_ratio, seed)

        train_embs = self._embeddings[train_idx]
        train_pt = self._prompt_tokens[train_idx]
        train_ct = self._completion_tokens[train_idx]
        test_embs = self._embeddings[test_idx]
        test_costs = self._costs[test_idx]
        ...
        for i in range(len(test_idx)):
            ...
            pred_pt = np.average(train_pt[top_k_idx], axis=0, weights=weights)
            pred_ct = np.average(train_ct[top_k_idx], axis=0, weights=weights)
            pred_cost = pred_pt * input_prices / 1e6 + pred_ct * output_prices / 1e6

            errors[i] = np.abs(pred_cost - test_costs[i])
